...
5. Visualizamos los contenedores iniciados
```
docker ps
```
...
c22ed04ab0f1   grupo01-frontend   "/docker-entrypoint.…"   19 seconds ago   Up 17 seconds   0.0.0.0:4200->80/tcp          grupo01-frontend-1
cdb24094797c   dpage/pgadmin4     "/entrypoint.sh"         19 seconds ago   Up 18 seconds   0.0.0.0:80->80/tcp, 443/tcp   pgadmin
79116093c7ec   grupo01-backend    "java -jar app.jar"      19 seconds ago   Up 18 seconds   0.0.0.0:8080->8080/tcp        grupo01-backend-1
52b47368ae58   postgres:14        "docker-entrypoint.s…"   19 seconds ago   Up 18 seconds   0.0.0.0:5432->5432/tcp        postgres
```

6. Obtenemos la IP del contenedor de postgres
```
docker inspect <id_del_contenedor>
```
Para la salida mostrada anteriormente, el id del contenedor de postgres es `52b47368ae58`. Podemos ejecutar el comando usando solamente los primeros caracteres del id. Por ejemplo:
...
...
            "Networks": {
                "grupo01_default": {
                    ...
                    "NetworkID": "4e1902bdcef9a18f5935c46316dcaf13fae09551b49f32e7785975a5fa7feff3",
                    "EndpointID": "08b93377e94c0e11953d472e1edb73fb32dd9d7c7dcd257908f621a87817d39f",
                    "Gateway": "172.19.0.1",
...
Ubicar la línea `"IPAddress": "172.19.0.2"`. Para este ejemplo, la dirección IP del contenedor de postgres es `172.19.0.2`.

7. Ingresar a PgAdmin (en el puerto `80`), con los siguientes datos:
* **Email Address / Username:** admin@admin.com
* **Password:** admin

8. Registrar un servidor en PgAdmin y conectarse. Usar cualquier nombre para el nombre del servidor. Utilizar los siguientes parámetros para la conexión (mantener el resto en sus valores por defecto):
* **Host name/address:** (la dirección IP hallada en el paso 6; ejemplo: `172.19.0.2`)
